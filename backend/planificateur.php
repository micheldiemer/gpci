...
$app->post('/plan/salle', $authenticateWithRole('planificateur'), function() use ($app) {
    try{
        ...
        $salle = new Salles;
        $salle->classe = $data['classe'];
        $salle->cour = $data['cour'];
        $salle->salle = $data['salle'];
        $salle->save();
    } catch(Exception $e) {
        $app->response->headers->set('Content-Type', 'application/json');
		$app->response->setStatus(400);
...
$app->delete('/plan/salle/:id', $authenticateWithRole('planificateur'), function($id) use ($app) {
    try{
        $cour = Salles::where("id",$id)->has('cours')->count();
        if($cour == 0){
             salles::find($id)->delete();
        }
        else{
            $app->response->headers->set('Content-Type', 'application/json');
            $app->response->setBody(json_encode(array("message"=>"Vous ne pouvez pas supprimer une salle avec des cours!")));
            $app->response->setStatus(400);
        }
