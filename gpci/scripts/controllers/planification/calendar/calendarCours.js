webApp.controller(
  ...
  function (
      ...
      $scope.$watch(
        ...
        function (value) {
          ...
          angular.forEach($scope.formClasses, function (v, k) {
            v &&
              $scope.cours.classes.push(
                helper.getObjectFromArray($scope.classes, "id", k)
              );
          });
        },
        true
    ...
    function filtreEnseignants(enseignant) {
      if (!$scope.cours.id_Matieres) return true;

      if (
        helper.getObjectFromArray(
          enseignant.matieres,
          "id",
          $scope.cours.id_Matieres
        )
      ) {
        return true;
      } else {
    ...
    function ajouterDisponibilité(enseignants) {
      angular.forEach(enseignants, function (enseignant) {
        if (enseignant.id == $scope.cours.id_Users) {
          ...
          helper.getPeriodFromDateInArray(
            enseignant.indisponibilite,
            event.start
          )
        ) {
