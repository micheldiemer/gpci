webApp.factory(
    ...
    function (
                ...
                Restangular.all("ens/indispo")
                    ...
                    .then(
                        function (data) {
                            ...
                            uiCalendarConfig.calendars.indisposCalendar.fullCalendar(
                                "prev"
                            );
                            uiCalendarConfig.calendars.indisposCalendar.fullCalendar(
                                "next"
        ...
        function checkDayForIndispo(date) {
            var result = 0;
            var indispo = helper.getObjectFromDateInArray(
                indispos,
                "start",
                date
            );
            if (indispo) {
                if (indispo.toDelete) {
                    ...
                    moment(indispo.start).hour() === 8 &&
                    moment(indispo.end).hour() === 17 &&
                    moment(indispo.start).isSame(indispo.end, "day")
                ) {
                    result = 1;
                } else if (
                    moment(indispo.start).hour() === 8 &&
                    moment(indispo.end).hour() === 12
                ) {
                    result = 2;
                } else if (
                    moment(indispo.start).hour() === 13 &&
                    moment(indispo.end).hour() === 17
                ) {
                    result = 3;
                } else {
        ...
        function changeIndispoType(date) {
            ...
            if (indispo) {
                ...
                if (indispo.toDelete) {
                    indispo.start = moment(indispo.start)
                        .hour(8)
                ...
                //Si Journée -> Matin
                    ...
                    indispo.end = moment(indispo.end)
                        .hour(12)
                        .minutes(15)
                        .format("YYYY-MM-DD HH:mm:ss");
                }
        ...
        function updateIndispos() {
            ...
            indispos.forEach(function (ind) {
                ...
                if (ind.toDelete) {
                    promises.push(
                        Restangular.one("ens/indispo", ind.id).remove()
                    );
                }
            });
            ...
            $q.all(promises).then(
                ...
                function () {
                    ...
                    listModified = false;
                    notifService.error(
                        "Un problème avec le serveur a empeché la sauvegarde"
                    );
                    return false;
                }
            );
        ...
        function openPeriodeModal(date) {
            var indispo = date
                ? helper.getPeriodFromDateInArray(indispos, date)
                : { isNew: true };
            ...
            modalInstance.result.then(
                function (newIndispo) {
                    if (newIndispo.toSave) {
                        ...
                        Restangular.one("ens/indispo", newIndispo.id)
                            .remove()
                            .then(function () {
                                getServerIndispos();
                            });
